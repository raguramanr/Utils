...
$globals->{mgrName}          = '';
$globals->{bgColor}    	     = '';
$globals->{emailSubject}     = 'Bug Report';
$globals->{releaseName}      = '';
$globals->{releaseId}        = 0;
...
{
  my $sqlStatement = qq(
SELECT bugDescriptions.bugNumber, bugDescriptions.severity, bugDescriptions.priority, bugDescriptions.summary, bugDescriptions.globalState, bugDescriptions.creator,ldapManagerName,
...
AND ldapManagerName like '%$globals->{mgrName}%'
GROUP BY bugDescriptions.bugNumber
ORDER BY bugDescriptions.globalState ASC, numDays DESC;
                      );
  my $queryRef     = $globals->{dbReference}->prepare($sqlStatement);
...
{
  my $sqlStatement = qq(
SELECT bugNumber, summary, severity, priority, releaseState, creator, relassignedTo,  DATE_FORMAT(MAX(transitionDate),'%e/%b/%Y') AS verifyFixDate,
...
AND verifierManager.ldapManagerName like '%$globals->{mgrName}%'
GROUP BY bugNumber
...
sub generateAndSendEmail()
{
  ...
  my $strMsg = qq(Hi all,<br><br>Please find below the CR Backlog details as on $globals->{today}<br><br>);
...
sub main()
{
  ...
  $globals->{mgrName}     = $ARGV[1];
  $globals->{sendEmailTo} = $ARGV[2];
  $globals->{sendEmailCc} = $ARGV[3];
  connectToDatabase();
  getReleaseId();
