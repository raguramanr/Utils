...
#$globals->{sendEmailTo}      = 'ajosephantilraj@extremenetworks.com,pragunathan@extremenetworks.com,tprasanna@extremenetworks.com,venshivaram@extremenetworks.com,anarayanan@extremenetworks.com,mvarakala@extremenetworks.com,rmasadi@extremenetworks.com,rouseph@extremenetworks.com,sasrinivasan@extremenetworks.com,vaswadhati@extremenetworks.com,pvaigundaboopathy@extremenetworks.com';
#$globals->{sendEmailCc}      = 'srbalakrishnan@extremenetworks.com,gramkumar@extremeNetworks.com,spalanivel@extremeNetworks.com,rvelusamy@extremenetworks.com,ragrajan@extremenetworks.com,uparthasarathy@extremenetworks.com';
$globals->{sendEmailTo}      = 'srbalakrishnan@extremenetworks.com';
$globals->{sendEmailCc}      = '';
$globals->{sendEmailFrom}    = 'RBC Mailer <srbalakrishnan@extremenetworks.com>';
$globals->{intbgColor}       = '';
$globals->{extbgColor}       = '';
...
WHERE ( bugDescriptions.productFamily = 'cqg' ) 
   ...
   OR ( bugDescriptions.creationTimeStamp >= '2018-02-01 00:00:00' )   
...
my $strMsg = qq(<font size="3" face="calibri"><i> Hello all,<br><br>Please find below FIT & SVT CQG details as on $globals->{today}.</i><br><br></font>);
...
<style>
  ...
  # <font size=3 color=red><i>Total CQGs - $globals->{cqgCRcount}</i><br></font>
  $strMsg .= qq(<font size=3 color=red><i>Total CQGs - $globals->{cqgAssignedCount} </i><br></font>
  <TABLE BORDER=1>
  <TR bgcolor=#4CAF50>
  ...
  foreach my $currentBug (@pendingCQGBugs) {
	...
	if ($currentBug->{metaDataFIT} ne "" && $currentBug->{metaDataSVT} ne "") { next }
	
    if ($currentBug->{bugDescriptions_priority} == "1 - Critical" 
			|| $currentBug->{bugDescriptions_priority} == "2 - Urgent") {
       $globals->{intSLA} = 30;
	...
	if ($currentBug->{bugDescriptions_subComponent} eq "DC") {
		...
		if ($currentBug->{bugDescriptions_module} eq "General Multicast"
			...
			|| $currentBug->{bugDescriptions_module} eq "IGMP"			
  ...
  <br><font size="3" face="calibri"><i> Thanks,<br>Sridhar </i></font> \n);
...
sub runCQGCount()
{
  ...
  foreach my $currentBug (@pendingCQGBugs) {
	...
	if ($globals->{FIT_Owner} ne "NA"
			|| $globals->{FIT_Owner} ne "NA") {
		$globals->{cqgAssignedCount} = $globals->{cqgAssignedCount} + 1
	}
  } 
